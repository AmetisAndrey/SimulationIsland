...
public class Simulation {
    ...
    public void start() {
        ...
        for (int i = 0; i < 100; i++) {
            island.randomPopulate(() -> AnimalFactory.randomAnimal(), 1);
        }
            ...
            if (count == 0) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    if (cell.canAddAnimal(type)) {
                        cell.addAnimal(AnimalFactory.create(type));
                    }
                }
            }
        }
        ...
        for (int i = 0; i < Island.HEIGHT; i++) {
            for (int j = 0; j < Island.WIDTH; j++) {
                ...
                predators += c.getAnimals().stream()
                        .filter(a -> a.isAlive() && a instanceof Predators)
                        .count();

                herbivores += c.getAnimals().stream()
                        .filter(a -> a.isAlive() && a instanceof Herbivores)
                        .count();

                omnivores += c.getAnimals().stream()
                        .filter(a -> a.isAlive() && a instanceof Omnivorous)
                        .count();
            }
        }
